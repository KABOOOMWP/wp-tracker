...
class PillDetectionEdgeCaseTest {
    ...
    @Test fun `Golden phase opp serves you return → GOLDEN_POINT (overrides break)`() {
    ...
    @Test fun `STAR_POINT opp serves you return → STAR_POINT (overrides break)`() {
    ...
    @Test fun `Golden phase you serve → GOLDEN_POINT (deciding phase always overrides break)`() {
    ...
    @Test fun `STAR_ADV_YOU you serve opp returning - HIDDEN (opp has no game point)`() {
    ...
    @Test fun `6-5 games, you have game point while serving → SET_POINT`() {
        val s = makeSnapshot(
    ...
    @Test fun `6-5 games, you returning with game point → SET_POINT (also break, but set wins priority)`() {
        val s = makeSnapshot(
            youGames = 6, oppGames = 5,
            youPoints = 3, oppPoints = 0,
    ...
    @Test fun `5-6 games, opp has game point → SET_POINT for opp`() {
        val s = makeSnapshot(
    ...
    @Test fun `5-5 games, you have game point → NOT set point (6-5 not enough)`() {
        val s = makeSnapshot(
    ...
    @Test fun `best-of-5 at 1-1 sets, returning, set point → SET_POINT`() {
        val s = makeSnapshot(
            config = singlesConfig(bestOf = 5),
    ...
    @Test fun `STAR_ADV_OPP opp serves you returning - HIDDEN (you have no game point)`() {
